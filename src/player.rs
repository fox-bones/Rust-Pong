...
impl Player {
    ...
    /// Keyboard listener — assigns a Direction based on arrow key input.
    ...
    /// Automatic movement — tracks the ball when it's heading toward the player's side.
    pub fn automatic_movement(&mut self, ball: &Ball) {
        let ball_pos = ball.get_position();
        let ball_vel_x = ball.get_velocity_x();

        if ball_vel_x < 0.0 && ball_pos.x <= screen_width() / 2.0 {
    ...
    /// Updates position based on the current direction.
